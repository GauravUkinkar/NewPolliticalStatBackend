...
public class ExcelExportServiceImpl implements ExcelExportService {
	...
	private static final String[] COLUMNS = { "Voter ID", "Voter Name (EN)", "Voter Name (Marathi)", "Father Name (EN)",
			"Father Name (Marathi)", "Age", "House No", "Gender", "Prabhag ID", "villageName"

	};

	@Override
	public void exportVoters(List<VotersDetails> voters, String fileName, HttpServletResponse response)
			throws IOException {
		...
		try (Workbook workbook = new XSSFWorkbook()) {
			...
			for (VotersDetails v : voters) {
				...
				row.createCell(8).setCellValue(v.getPrabhag().getVillageName());
